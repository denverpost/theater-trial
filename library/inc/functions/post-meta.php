...
if ( !function_exists('reactor_post_meta') ) {
	function reactor_post_meta( $args = '' ) {
		...
		if ( $args['date_only'] && function_exists( 'ap_time' ) ) {
			$classes = ' ' . implode( ' ', get_post_class() );
			$date = sprintf('<div class="entry-meta-date%1$s"><time class="entry-date" datetime="%2$s" pubdate><span class="entry-date-date">%3$s</span><span class="entry-date-time">%4$s</span></time><div class="clear"></div></div>',
				( strlen( $classes ) > 1 ) ? $classes : '',
				esc_attr( get_the_date('c') ),
				esc_html( get_ap_date() ),
				esc_html( get_ap_time() )
			);
		} else {
		...
		if ( $date || $categories_list || $author || $tag_list ) {
			if ( $args['show_icons'] ) {
				...
				//$meta .= ( is_single() ) ? '<span class="single-comments-link">%9$s</span>' : ''; // testing something else
